...
UBandLabelFactory::UBandLabelFactory(const FObjectInitializer &ObjectInitializer)
...
UObject* UBandLabelFactory::FactoryCreateFile(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	FEditorDelegates::OnAssetPreImport.Broadcast(this, InClass, InParent, InName, Parms);
	UObject* Label = UBandLabel::LoadLabel(InParent, InName, Flags, Filename, bOutOperationCanceled);
	if (!Label)
	{
