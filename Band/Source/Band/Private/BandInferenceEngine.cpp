...
void ABandInferenceEngine::BeginPlay()
{
	if (Model && Label)
	{
		if (UIWidgetReference)
		{
			...
			ClassTextBlock = Cast<UTextBlock>(UIWidget->GetWidgetFromName("ClassLabel"));
			MainTextBlock = Cast<UTextBlock>(UIWidget->GetWidgetFromName("MainThread"));
			RenderTextBlock = Cast<UTextBlock>(UIWidget->GetWidgetFromName("RenderThread"));
			DNNTextBlock = Cast<UTextBlock>(UIWidget->GetWidgetFromName("DNNThread"));
		}

		OnFrameHandle = CameraComponent->OnFrameAvailable.AddUObject(this, &ABandInferenceEngine::OnFrameAvailable);
		OnBeginFrameHandle = FCoreDelegates::OnBeginFrame.AddUObject(this, &ABandInferenceEngine::OnBeginFrame);
		OnEndFrameHandle = FCoreDelegates::OnEndFrame.AddUObject(this, &ABandInferenceEngine::OnEndFrame);
		OnBeginFrameRTHandle = FCoreDelegates::OnBeginFrameRT.AddUObject(this, &ABandInferenceEngine::OnBeginFrameRT);
...
void ABandInferenceEngine::OnEndFrame()
{
	if (MainTextBlock)
	{
		double FrameTime = FPlatformTime::ToMilliseconds64(FPlatformTime::Cycles64() - BeginFrame);
		...
		{
			double CameraTime = FPlatformTime::ToMilliseconds64(FPlatformTime::Cycles64() - BeginCameraFrame);
			AsyncTask(ENamedThreads::GameThread, [&, CameraTime]() {
				DNNTextBlock->SetText(FText::FromString(FString::Printf(TEXT("DNN processing thread %f ms"), CameraTime)));
			});
...
void ABandInferenceEngine::OnEndFrameRT()
{
	if (RenderTextBlock)
	{
		double FrameTime = FPlatformTime::ToMilliseconds64(FPlatformTime::Cycles64() - BeginFrameRT);
		AsyncTask(ENamedThreads::GameThread, [&, FrameTime]() {
			RenderTextBlock->SetText(FText::FromString(FString::Printf(TEXT("Render thread %f ms"), FrameTime)));
		});
	}
}
