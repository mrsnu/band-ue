...
namespace Band
{
  ...
  public:
    ...
    struct YuvData
    {
      const uint8 *y_buffer = nullptr;
      const uint8 *u_buffer = nullptr;
      const uint8 *v_buffer = nullptr;
      ...
      operator bool() const
      {
        return y_buffer != nullptr && u_buffer != nullptr && v_buffer != nullptr;
      }
    };
    ...
    static std::unique_ptr<FrameBuffer> Create(const std::vector<Plane> &planes,
                                               ...
                                               uint64 timestamp)
    {
      return std::make_unique<FrameBuffer>(planes, dimension, format,
    ...
    static std::unique_ptr<FrameBuffer> Create(std::vector<Plane> &&planes,
                                               ...
                                               uint64 timestamp)
    {
      return std::make_unique<FrameBuffer>(std::move(planes), dimension, format,
                                           orientation, timestamp);
    }
    ...
    {
      return std::make_unique<FrameBuffer>(planes, dimension, format,
                                           orientation, FPlatformTime::Cycles64());
    }
    ...
    static YuvData GetYuvDataFromFrameBuffer(
    ...
    FrameBuffer(const std::vector<Plane> &planes, Dimension dimension,
                Format format, Orientation orientation, uint64 timestamp)
        : planes_(planes),
    ...
    FrameBuffer(std::vector<Plane> &&planes, Dimension dimension, Format format,
                Orientation orientation, uint64 timestamp)
