...
namespace Band
{
  ...
  namespace
  {
                                         ...
                                         FrameBuffer::Dimension frame_dimension)
    {
      ...
      BoundingBox result{rx, ry, rw, rh};
      return result;
    }
  ...
  FrameBufferUtils::FrameBufferUtils(ProcessEngine engine)
  {
    ...
    case ProcessEngine::kLibyuv:
      utils_ = std::make_unique<LibyuvFrameBufferUtils>();
      break;
    default:
      UE_LOG(LogBand, Error, TEXT("Unexpected ProcessEngine: %d"), engine);
    }
  }
      ...
      FrameBuffer::Dimension from_dimension)
  {
    BoundingBox from_box{
        (int)from_left * from_dimension.width,
        (int)from_top * from_dimension.height,
        (int)round(abs(from_right - from_left) * from_dimension.width),
        (int)round(abs(from_bottom - from_top) * from_dimension.height)};
                               ...
                               FrameBuffer::Orientation to_orientation)
  {
    ...
    int angle = 0;
    OrientParams::FlipType flip = OrientParams::FlipType::kNo;

    check(from_index > -1 && to_index > -1);
  ...
  bool FrameBufferUtils::Crop(const FrameBuffer &buffer, int x0, int y0,
                              int x1, int y1,
                              FrameBuffer *output_buffer)
  {
    check(utils_ != nullptr);
      ...
      const FrameBuffer &buffer, const FrameBufferOperation &operation)
  {
    ...
    if (operation.IsType() == FrameBufferOperation::Type::kOrient)
    {
      OrientParams params =
          GetOrientParams(buffer.orientation(),
                          static_cast<const OrientOperation *>(&operation)->to_orientation);
      if (params.rotation_angle_deg == 90 || params.rotation_angle_deg == 270)
      {
        dimension.Swap();
      }
    }
    else if (operation.IsType() == FrameBufferOperation::Type::kCropResize)
    {
      const auto &crop_resize = static_cast<const CropResizeOperation *>(&operation);
      dimension = crop_resize->resize_dimension;
    }
    else if (operation.IsType() == FrameBufferOperation::Type::kUniformCropResize)
    {
      const auto &uniform_crop_resize =
          static_cast<const UniformCropResizeOperation *>(&operation);
      ...
      const FrameBuffer &buffer, const FrameBufferOperation &operation)
  {
    if (operation.IsType() == FrameBufferOperation::Type::kOrient)
    {
      return static_cast<const OrientOperation *>(&operation)->to_orientation;
    }
  ...
  FrameBuffer::Format FrameBufferUtils::GetFormat(
      const FrameBuffer &buffer, const FrameBufferOperation &operation)
  {
    if (operation.IsType() == FrameBufferOperation::Type::kConvert)
    {
      return static_cast<const ConvertOperation *>(&operation)->to_format;
    }
    return buffer.format();
  }

  bool FrameBufferUtils::Execute(const FrameBuffer &buffer,
                                 const FrameBufferOperation &operation,
                                 FrameBuffer *output_buffer)
  {
    if (operation.IsType() == FrameBufferOperation::Type::kCropResize)
    {
      const auto &params = static_cast<const CropResizeOperation *>(&operation);
      RETURN_IF_ERROR(
    ...
    {
      const auto &params = static_cast<const UniformCropResizeOperation *>(&operation);
      RETURN_IF_ERROR(
          Crop(buffer, params->crop_origin_x, params->crop_origin_y,
               (params->crop_dimension.width + params->crop_origin_x - 1),
               (params->crop_dimension.height + params->crop_origin_y - 1),
    ...
    else if (operation.IsType() == FrameBufferOperation::Type::kConvert)
    {
      RETURN_IF_ERROR(Convert(buffer, output_buffer));
    }
    else if (operation.IsType() == FrameBufferOperation::Type::kOrient)
    {
      RETURN_IF_ERROR(Orient(buffer, output_buffer));
    }
    else
    {
      UE_LOG(LogBand, Error, TEXT("FrameBufferOperation is not supported"));
      return false;
    }
    return true;
  }

  bool FrameBufferUtils::Resize(const FrameBuffer &buffer,
                                FrameBuffer *output_buffer)
  {
    check(utils_ != nullptr);
  ...
  bool FrameBufferUtils::Rotate(const FrameBuffer &buffer,
  ...
  bool FrameBufferUtils::FlipHorizontally(const FrameBuffer &buffer,
                                          FrameBuffer *output_buffer)
  {
    check(utils_ != nullptr);
  ...
  bool FrameBufferUtils::FlipVertically(const FrameBuffer &buffer,
                                        FrameBuffer *output_buffer)
  {
    check(utils_ != nullptr);
  ...
  bool FrameBufferUtils::Convert(const FrameBuffer &buffer,
                                 FrameBuffer *output_buffer)
  {
    check(utils_ != nullptr);
  ...
  bool FrameBufferUtils::Orient(const FrameBuffer &buffer,
                                FrameBuffer *output_buffer)
  {
    check(utils_ != nullptr);
    ...
    if (params.rotation_angle_deg == 0 && params.flip == OrientParams::FlipType::kNo)
    {
    ...
    if (params.flip == OrientParams::FlipType::kNo)
    {
    ...
    auto tmp_buffer = std::make_unique<uint8[]>(
      ...
      FrameBuffer *output_buffer)
  {
    ...
    for (int i = 0; i < operations.size(); i++)
    {
      ...
      if ((i + 1) == operations.size())
      {
        ...
        {
          UE_LOG(LogBand, Error, TEXT("The output metadata does not match pipeline result metadata"));
          return false;
        }
      }
      else
      {
        ...
        if (i % 2 == 0)
        {
          if (buffer1_size < byte_size)
          {
            ...
            buffer1 = std::make_unique<uint8[]>(byte_size);
          }
        ...
        else
        {
          if (buffer2_size < byte_size)
          {
            ...
            buffer2 = std::make_unique<uint8[]>(byte_size);
          }
          planes = GetPlanes(buffer2.get(), new_size, new_format);
        }
        if (planes.empty())
        {
          UE_LOG(LogBand, Error, TEXT("Failed to construct temporary buffer"));
          return false;
        }
  ...
  bool FrameBufferUtils::Preprocess(
      const FrameBuffer &buffer, BoundingBox bounding_box,
      FrameBuffer *output_buffer, bool uniform_resizing)
  {
    ...
    if (uniform_resizing)
    {
      ...
      frame_buffer_operations.push_back(new UniformCropResizeOperation(
          bounding_box.origin_x, bounding_box.origin_y,
          FrameBuffer::Dimension{bounding_box.width,
                                 bounding_box.height},
    ...
    else if (pre_orient_dimension != buffer.dimension())
    {
      ...
      frame_buffer_operations.push_back(
          new CropResizeOperation(0, 0, buffer.dimension(), pre_orient_dimension));
    }
        ...
        buffer.format() == FrameBuffer::Format::kRGBA)
    {
      ...
      {
        frame_buffer_operations.push_back(
            new ConvertOperation(output_buffer->format()));
      }
      // Handle orientation conversion
      if (output_buffer->orientation() != buffer.orientation())
      {
        frame_buffer_operations.push_back(
            new OrientOperation(output_buffer->orientation()));
      }
    }
    else
    {
  ...
  bool FrameBufferUtils::Preprocess(const FrameBuffer &buffer, FrameBuffer *output_buffer, bool uniform_resizing)
  {
    std::vector<FrameBufferOperation *> frame_buffer_operations;
    // Handle cropping and resizing.
    bool needs_dimension_swap =
