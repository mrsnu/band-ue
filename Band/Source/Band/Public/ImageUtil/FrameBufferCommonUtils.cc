...
namespace Band
{
  ...
  int GetPixelStrides(FrameBuffer::Format format)
  {
    ...
    default:
      UE_LOG(LogBand, Display, TEXT("GetPixelStrides does not support format: %i"), format);
      return -1;
    }
  }
  ...
  const uint8 *GetUvRawBuffer(const FrameBuffer &buffer)
  {
    ...
    {
      UE_LOG(LogBand, Display, TEXT("Only support getting biplanar UV buffer from NV12/NV21 frame buffer"));
      return nullptr;
    }
      ...
      FrameBuffer::Dimension dimension, FrameBuffer::Format format)
  {
    ...
    {
      UE_LOG(LogBand, Display, TEXT("Invalid input dimension: {%d, %d}"), dimension.width,
             dimension.height);
      return {};
    }
    switch (format)
  ...
  bool ValidateBufferFormat(const FrameBuffer &buffer)
  {
    ...
    case FrameBuffer::Format::kRGBA:
      if (buffer.plane_count() == 1)
        return true;
      UE_LOG(LogBand, Display, TEXT("Plane count must be 1 for grayscale and RGB[a] buffers"));
      return false;
    case FrameBuffer::Format::kNV21:
    case FrameBuffer::Format::kNV12:
    case FrameBuffer::Format::kYV21:
    case FrameBuffer::Format::kYV12:
      return true;
    default:
      UE_LOG(LogBand, Display, TEXT("Unsupported buffer format: %i"), buffer.format());
      return false;
    }
  }
                                  ...
                                  const FrameBuffer &output_buffer)
  {
    ...
    if (!valid_format)
    {
      UE_LOG(LogBand, Display, TEXT("Input and output buffer formats must match"));
      return false;
    }
                                  ...
                                  int angle_deg)
  {
    ...
    {
      UE_LOG(LogBand, Display, TEXT("Rotation angle must be between 0 and 360, in multiples of 90 degrees"));
      return false;
    }
    else if ((is_dimension_change && !are_dimensions_rotated) ||
             (!is_dimension_change && !are_dimensions_equal))
    {
      UE_LOG(LogBand, Display, TEXT("Output buffer has invalid dimensions for rotation"));
      return false;
    }
    return true;
  }
                              ...
                              FrameBuffer::Format to_format)
  {
    ...
    {
    case FrameBuffer::Format::kGRAY:
      UE_LOG(LogBand, Display, TEXT("Grayscale format does not convert to other formats"));
      return false;
    case FrameBuffer::Format::kRGB:
    ...
    case FrameBuffer::Format::kYV21:
      return true;
    default:
      UE_LOG(LogBand, Display, TEXT("Unsupported buffer format: %i"), from_format);
      return false;
    }
  }
      ...
      FrameBuffer::Orientation orientation, uint64 timestamp)
  {
    ...
    default:
      ...
      UE_LOG(LogBand, Display, TEXT("Unsupported buffer format: %i"), target_format);
      return nullptr;
    }
  }
