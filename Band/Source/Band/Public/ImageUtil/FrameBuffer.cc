...
namespace Band
{
  namespace
  {
    ...
    FrameBuffer::YuvData GetYuvDataFromOnePlaneFrameBuffer(
        const FrameBuffer &source)
    {
      if (!IsSupportedYuvFormat(source.format()))
      {
        UE_LOG(LogBand, Display, TEXT("The source FrameBuffer format is not part of YUV420 family"));
        return {};
      }
    ...
    FrameBuffer::YuvData GetYuvDataFromTwoPlaneFrameBuffer(
        const FrameBuffer &source)
    {
      if (source.format() != FrameBuffer::Format::kNV12 &&
          source.format() != FrameBuffer::Format::kNV21)
      {
        UE_LOG(LogBand, Display, TEXT("Unsupported YUV planar format"));
        return {};
      }
    ...
    FrameBuffer::YuvData GetYuvDataFromThreePlaneFrameBuffer(
        const FrameBuffer &source)
    {
      if (!IsSupportedYuvFormat(source.format()))
      {
      ...
      const FrameBuffer &source)
  {
    ...
    UE_LOG(LogBand, Display, TEXT("The source FrameBuffer must be consisted by 1, 2, or 3 planes"));
    return {};
  }
