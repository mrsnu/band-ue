...
template <typename T>
FString GetLabelInternal(const TArray<T> &OutputTensor,
...
void UBandBlueprintLibrary::RequestSync(
    UPARAM(ref) const UBandModel *Model,
    UPARAM(ref) TArray<UBandTensor *> InputTensors,
    UPARAM(ref) TArray<UBandTensor *> OutputTensors, int WorkerIndex) {
  if ((InputTensors.Num() == 0 || OutputTensors.Num() == 0)) {
    UE_LOG(LogBand, Error,
           TEXT("InvokeSync: Failed to invoke. Please check that the "
                "input/output tensors are not empty"));
  } else if (Model->GetInputTensorCount() == InputTensors.Num() &&
             Model->GetOutputTensorCount() == OutputTensors.Num()) {
    FBandModule::Get().RequestSync(Model, InputTensors, OutputTensors);
  } else {
    UE_LOG(LogBand, Error,
           TEXT("InvokeSync: Failed to invoke. Input Count (Expected %d, Given "
                "%d) Output Count (Expected %d, Given %d)"),
           Model->GetInputTensorCount(), InputTensors.Num(),
           Model->GetOutputTensorCount(), OutputTensors.Num());
  }
}

int32 UBandBlueprintLibrary::RequestAsync(
    UPARAM(ref) const UBandModel *Model,
    UPARAM(ref) TArray<UBandTensor *> InputTensors, int WorkerIndex) {
  if (InputTensors.Num() == 0) {
    UE_LOG(LogBand, Error,
           TEXT("InvokeSync: Failed to invoke. Please check that the input "
                "tensors are not empty"));

    return -1;
  } else if (Model->GetInputTensorCount() == InputTensors.Num()) {
    return FBandModule::Get().RequestAsync(Model, InputTensors);
  } else {
    UE_LOG(LogBand, Error,
           TEXT("InvokeSync: Failed to invoke. Input Count (Expected %d, Given "
                "%d)"),
...
EBandStatus UBandBlueprintLibrary::GetOutputs(
    int32 JobId, UPARAM(ref) TArray<UBandTensor *> OutputTensors) {
  return static_cast<EBandStatus>(
      FBandModule::Get().GetOutputs(JobId, OutputTensors));
}

int32 UBandBlueprintLibrary::GetNumWorkers() {
  return FBandModule::Get().GetNumWorkers();
}

EBandDeviceType UBandBlueprintLibrary::GetWorkerDeviceType(int Index) {
  return FBandModule::Get().GetDeviceType(Index);
}
...
FString UBandBlueprintLibrary::GetLabel(TArray<UBandTensor *> OutputTensors,
  ...
  case EBandTensorDataType::Float32:
    return GetLabelInternal<float>(OutputTensors[0]->GetF32Buffer(), Label);
  case EBandTensorDataType::Int8:
    return GetLabelInternal<int8>(OutputTensors[0]->GetBuffer<int8>(), Label);
  case EBandTensorDataType::UInt8:
    return GetLabelInternal<uint8>(OutputTensors[0]->GetUInt8Buffer(), Label);
  default:
    UE_LOG(LogBand, Error,
...
TArray<FBandBoundingBox> GetDetectedBoxesInternal(
    EBandDetector DetectorType, TArray<UBandTensor *> Tensors,
    const size_t DetectionTensorIndex, TArray<int32> BBoxOffsets,
    const size_t ConfidenceTensorIndex, const size_t ConfidenceOffset,
    const size_t ClassTensorIndex, const size_t ClassOffset,
    const size_t LandmarkTensorIndex, const size_t LandmarkOffset,
    const UBandLabel *Label, const float ScoreThreshold, int32 LenBoxVector,
...
TArray<FBandBoundingBox> UBandBlueprintLibrary::GetDetectedBoxes(
  ...
  case EBandTensorDataType::Float32:
    Boxes = GetDetectedBoxesInternal<float>(
        DetectorType, Tensors, DetectionTensorIndex, BBoxOffsets,
        ConfidenceTensorIndex, ConfidenceOffset, ClassTensorIndex, ClassOffset,
        LandmarkTensorIndex, LandmarkOffset, Label, ScoreThreshold,
        LenBoxVector, LenConfidenceVector, LenClassVector, LenLandmarkVector);
    break;
  default:
    const UEnum *EnumPtr =
...
template <typename T>
TArray<FVector2D>
Get2DLandmarksInternal(TArray<UBandTensor *> Tensors, int LandmarkTensorIndex,
                       int TensorDim, int NumLandmarks, TArray<int32> Offsets
...
TArray<FVector2D>
UBandBlueprintLibrary::Get2DLandmarks(TArray<UBandTensor *> Tensors,
  ...
  switch (TensorType) {
  case EBandTensorDataType::Float32:
    Landmarks = Get2DLandmarksInternal<float>(Tensors, LandmarkTensorIndex,
                                              TensorDim, NumLandmarks, Offsets);
    break;
  default:
    const UEnum *EnumPtr =
        FindObject<UEnum>(ANY_PACKAGE, TEXT("EBandTensorDataType"), true);
...
template <typename T>
FBandBoundingBox
GetLandmarksInternal(TArray<UBandTensor *> Tensors, int LandmarkTensorIndex,
                     int TensorDim, int NumLandmarks, int NumCoords,
...
FBandBoundingBox
UBandBlueprintLibrary::GetLandmarks(TArray<UBandTensor *> Tensors,
                                    EBandLandmark ModelType) {
  FBandBoundingBox Landmarks = {0, {0, 0, 0, 0}};
  int LandmarkTensorIndex = 0;
  int NumLandmarks = 0;
  ...
  case EBandTensorDataType::Float32:
    Landmarks = GetLandmarksInternal<float>(Tensors, LandmarkTensorIndex,
                                            TensorDim, NumLandmarks, NumCoords,
                                            Offsets, ConfOffset);
    break;
  default:
    const UEnum *EnumPtr =
  ...
  if (ModelType == EBandLandmark::MoveNetSingleThunder ||
      ModelType == EBandLandmark::MoveNetSingleLightning) {
    Landmarks.LandmarkEdge = {{0, 1},   {0, 2},   {1, 3},  {2, 4},   {0, 5},
    ...
    Landmarks.LandmarkEdge = {
        {0, 267},   {7, 163},   {10, 338},  {13, 312},  {14, 317},  {17, 314},
        {21, 54},   {33, 7},    {33, 246},  {37, 0},    {39, 37},   {40, 39},
        {46, 53},   {52, 65},   {53, 52},   {54, 103},  {58, 132},  {61, 146},
        {61, 185},  {63, 105},  {65, 55},   {66, 107},  {67, 109},  {70, 63},
        {78, 95},   {78, 191},  {80, 81},   {81, 82},   {82, 13},   {84, 17},
        {87, 14},   {88, 178},  {91, 181},  {93, 234},  {95, 88},   {103, 67},
        {105, 66},  {109, 10},  {127, 162}, {132, 93},  {136, 172}, {144, 145},
        {145, 153}, {146, 91},  {148, 176}, {149, 150}, {150, 136}, {152, 148},
        {153, 154}, {154, 155}, {155, 133}, {157, 173}, {158, 157}, {159, 158},
        {160, 159}, {161, 160}, {162, 21},  {163, 144}, {172, 58},  {173, 133},
        {176, 149}, {178, 87},  {181, 84},  {185, 40},  {191, 80},  {234, 127},
        {246, 161}, {249, 390}, {251, 389}, {263, 249}, {263, 466}, {267, 269},
        {269, 270}, {270, 409}, {276, 283}, {282, 295}, {283, 282}, {284, 251},
        {288, 397}, {293, 334}, {295, 285}, {296, 336}, {297, 332}, {300, 293},
        {310, 415}, {311, 310}, {312, 311}, {314, 405}, {317, 402}, {318, 324},
        {321, 375}, {323, 361}, {324, 308}, {332, 284}, {334, 296}, {338, 297},
        {356, 454}, {361, 288}, {365, 379}, {373, 374}, {374, 380}, {375, 291},
        {377, 152}, {378, 400}, {379, 378}, {380, 381}, {381, 382}, {382, 362},
        {384, 398}, {385, 384}, {386, 385}, {387, 386}, {388, 387}, {389, 356},
        {390, 373}, {397, 365}, {398, 362}, {400, 377}, {402, 318}, {405, 321},
...
TArray<FBandBoundingBox>
UBandBlueprintLibrary::NMS(TArray<FBandBoundingBox> Boxes,
...
TArray<FBandBoundingBox>
UBandBlueprintLibrary::FilterBoxesByConfidence(TArray<FBandBoundingBox> Boxes,
...
TArray<FBandBoundingBox>
UBandBlueprintLibrary::FilterBoxesByLabel(TArray<FBandBoundingBox> Boxes,
                                          FString Label) {
