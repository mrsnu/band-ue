  ...
  if (operation.IsType() == FrameBufferOperation::Type::kOrient) {
    OrientParams params =
        GetOrientParams(buffer.orientation(),
                        static_cast<const OrientOperation*>(&operation)->
                        to_orientation);
    if (params.rotation_angle_deg == 90 || params.rotation_angle_deg == 270) {
      dimension.Swap();
    }
  } else if (operation.IsType() == FrameBufferOperation::Type::kCropResize) {
    const auto& crop_resize = static_cast<const CropResizeOperation*>(&
      operation);
    dimension = crop_resize->resize_dimension;
  } else if (operation.IsType() ==
             FrameBufferOperation::Type::kUniformCropResize) {
    ...
    RETURN_IF_ERROR(
        Crop(buffer, params->crop_origin_x, params->crop_origin_y,
          ...
          output_buffer));
             ...
             FrameBufferOperation::Type::kUniformCropResize) {
    const auto& params = static_cast<const UniformCropResizeOperation*>(&
      operation);
    RETURN_IF_ERROR(
        Crop(buffer, params->crop_origin_x, params->crop_origin_y,
          (params->crop_dimension.width + params->crop_origin_x - 1),
...
bool FrameBufferUtils::Orient(const FrameBuffer& buffer,
  ...
  if (params.rotation_angle_deg == 0 && params.flip ==
      OrientParams::FlipType::kNo) {
...
bool FrameBufferUtils::Preprocess(
  ...
  if (uniform_resizing) {
    ...
    frame_buffer_operations.push_back(
        new CropResizeOperation(0, 0, buffer.dimension(),
                                pre_orient_dimension));
  }
...
bool FrameBufferUtils::Preprocess(const FrameBuffer& buffer,
                                  FrameBuffer* output_buffer,
